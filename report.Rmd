...
ggplot()+
  ...
  geom_text_repel(data = exp %>% filter(year==2021),
...
d <- cod21 %>%
  filter(DIM_COUNTRY_CODE == country_iso & FLAG_LEVEL==1 & sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
...
cod <- cod21 %>% 
  filter(DIM_COUNTRY_CODE == country_iso & FLAG_RANKABLE==1 & DIM_AGEGROUP_CODE=="TOTAL" & sex!="Both sexes") %>% 
...
d3 <- cod21 %>% 
  filter(DIM_COUNTRY_CODE == country_iso & FLAG_LEVEL==1 & sex %in% c("Male", "Female") & DIM_AGEGROUP_CODE=="TOTAL") %>%
  select(DIM_COUNTRY_CODE, country, sex, DIM_GHECAUSE_TITLE, VAL_DALY_COUNT_NUMERIC, ATTR_POPULATION_NUMERIC) %>% 
  rbind(cod21 %>%
          filter(region == who_region & FLAG_LEVEL==1 & sex %in% c("Male", "Female") & DIM_AGEGROUP_CODE=="TOTAL") %>%
            ...
            ungroup() %>% rename(country = region2) %>% mutate(DIM_COUNTRY_CODE=who_region)) %>%
...
ggplot() + 
  ...
  geom_col(data = d3 %>% filter(DIM_COUNTRY_CODE== country_iso),
  ...
  theme(
      strip.background = element_blank(),
      plot.margin = unit(c(0,0.5,0,-2.4), "cm"),
...
# config_all <- config::get("input/all.yml",
...
m = month(Sys.Date(), label=TRUE, , abbr = FALSE)
y = year(Sys.Date())
...
data <- data.frame(
  ...
  name = c("UNDESA population division, World Population Prospects 2022 \nExtracted: December 2024",
            ...
            paste("World Health Organization, Global Health Estimates \nExtracted:", m, y), # December 2024",
            paste("World Health Organization, Global Health Estimates \nExtracted:", m, y), # December 2024",
            paste("World Health Organization, Global Health Estimates, Burden of Disease \nExtracted:", m, y), # \nExtracted: December 2024",
