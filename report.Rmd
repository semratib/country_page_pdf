...
- \fancyhf{}
- "\\fancyhead[L]{\\color{whodarkblue} Health data overview   |   `r country_`} % Add
...
\vspace{1.5cm}
{\fontsize{25}{30}\selectfont \color{wholightblueshade} `r toupper(country_)`}
...
# total population
total_pop2023 <- as.numeric(pop %>% filter(country == country_ & year==2023) %>% pull(totalpop_jul))
total_pop2050 <- as.numeric(pop %>% filter(country == country_ & year==2050) %>% pull(totalpop_jul))
total_pop_increase <- round(((as.numeric(total_pop2050) - as.numeric(total_pop2023)) / as.numeric(total_pop2023))*100, digits = 0)

# life expectany
le_2000 <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2000 & country == country_) %>% pull(NumericValue)
le_2000_lwr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2000 & country == country_) %>% pull(Low)
le_2000_upr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2000 & country == country_) %>% pull(High)

le_2019 <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2019 & country == country_) %>% pull(NumericValue)
le_2019_lwr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2019 & country == country_) %>% pull(Low)
le_2019_upr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2019 & country == country_) %>% pull(High)

le_2021 <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & country == country_) %>% pull(NumericValue)
le_2021_lwr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & country == country_) %>% pull(Low)
le_2021_upr <- lehale %>% filter(IndicatorCode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & country == country_) %>% pull(High)
...
In `r country_`, the population as of 2023 is `r format(total_pop2023*1000, big.mark = " ")` with a projected increase of `r total_pop_increase`\% to `r format(total_pop2050*1000, big.mark = " ")` by 2050.
\\
\\
Life expectancy at birth (years) has `r change_text` `r round(change,1)` years from `r round(le_2000,1)` [`r round(le_2000_lwr,1)` - `r round(le_2000_upr,1)`] years in 2000 to `r round(le_2019,1)` [`r round(le_2019_lwr,1)` - `r round(le_2019_upr,1)`] years in 2019. Since 2019, life expectancy has `r change_text2` to `r round(le_2021,1)` [`r round(le_2021_lwr,1)` - `r round(le_2021_upr,1)`] years.
\\
\\
...
\normalsize Life expectancy and healthy life expectancy by sex, 2000–2021
...
\
\
Top 10 level 2 causes of death, 2000-2021
...
\
\
\normalsize Top 10 detailed causes of death by sex, 2021
\small   (deaths per 100 000 population)
```{r cod2021, fig.height=4, fig.align='center', fig.width=8}
cod <- cod21 %>% 
  filter(country==country_ & FLAG_TREEMAP==1 & DIM_AGEGROUP_CODE=="TOTAL" & sex!="Both sexes") %>% 
...
hstat <- rbind(
  sdg_hstat %>% group_by(IND_UUID) %>% filter(DIM_TIME<=2024) %>% top_n(1, DIM_TIME) %>% mutate(year = "new"),
...
hstat_final <- hstat %>% left_join(hstat_) %>% arrange(sort)
  
ind_hstat <- hstat_final %>% 
  # filter(graph==1) %>% 
  # arrange(row) %>%
  distinct(IND_UUID) %>%  pull(IND_UUID)

yearchange <- hstat %>% 
  select(IND_UUID, year, DIM_TIME) %>% 
  pivot_wider(names_from = year, values_from = DIM_TIME) %>% 
  mutate(flag_onedata = ifelse(new==old, 1, 0))

adjusth <- hstat %>% 
  select(IND_UUID, year, VALUE_NUMERIC) %>% 
  pivot_wider(names_from = year, values_from = VALUE_NUMERIC) %>% 
  left_join(yearchange %>% select(flag_onedata)) %>% 
  mutate(flag_nochange = ifelse(new==old & flag_onedata==0, 1, 0))
...
for (i in ind_hstat){
  ...
  plot_num <- hstat_final %>% filter(IND_UUID==i) %>% distinct(row) %>% pull(row); message(plot_num)
  flag <- yearchange %>% filter(IND_UUID==i) %>% pull(flag_onedata)
  flagh <- adjusth %>% filter(IND_UUID==i) %>% pull(flag_nochange)
  data_i <- hstat_final %>% filter(IND_UUID==i) %>% select(-(year)) %>% distinct()
  ...
  # assign(paste0("hsp_", plot_num), plot)
  
  hs_plots[[plot_num]] <- plot
}
n_hs_plots <- length(hs_plots)

if(n_hs_plots>6){include_hs_2nd = T}else{include_hs_2nd = F}

if(n_hs_plots>=6){
  wrap_plots(hs_plots[1:6], ncol = 2)
}else{
  wrap_plots(hs_plots[1:n_hs_plots], ncol = 2)
}
```

```{r health status2, fig.height=10.3, fig.width = 8, fig.align='center', message=F, warning=F, include = include_hs_2nd}
if(n_hs_plots>12){include_hs_3rd = T}else{include_hs_3rd = F}

if(n_hs_plots>=12){
  wrap_plots(hs_plots[7:12], ncol = 2)
}else{
  wrap_plots(hs_plots[7:n_hs_plots], ncol = 2)
}
```

```{r health status3, fig.height=3.5, fig.width = 8, fig.align='center', message=F, warning=F, include = include_hs_3rd}
wrap_plots(hs_plots[13:n_hs_plots], ncol = 2)
```
\newpage
...
risk <- rbind(
  sdg_risk %>% group_by(IND_UUID) %>% filter(DIM_TIME<=2024) %>% top_n(1, DIM_TIME) %>% mutate(year = "new"),
...
risk_final <- risk %>% left_join(risk_) %>% arrange(sort)
...
for (i in ind_risk){
  ...
  message(i)
  name <- sdg_ref %>% filter(IND_UUID==i) %>% pull(website_name) ; message(name)
  plot_num <- risk_final %>% filter(IND_UUID==i) %>% distinct(row) %>% pull(row); message(plot_num)
  flag <- yearchange %>% filter(IND_UUID==i) %>% pull(flag_onedata)
  data_i <- risk_final %>% filter(IND_UUID==i) %>% select(-(year)) %>% distinct()
...
```{r risk factors2, fig.height=10.3, fig.width = 8, fig.align='center', message=F, warning=F, include = include_rk_2nd}
...
```{r risk factors3, fig.height=3.5, fig.width = 8, fig.align='center', message=F, warning=F, include = include_rk_3rd}
if(n_rk_plots>12){
...
sc_final <- sc %>% left_join(sc_) %>% arrange(sort)
...
hs_final <- hs %>% left_join(hs_) %>% arrange(sort)
...
\newpage
\normalsize `r who_region_name`: Triple Billions indicator progress – likelihood of achieving by 2030
...
bh_iso <- ggplot(indicator_values %>% filter(aggregate_id==country_iso), 
       aes(x=indicator_name, y=str_wrap(country, 25), fill=ach, label=ach, family="calibri")) +
...
dt.ft <- fontsize(dt.ft, i = 1:7, j = 1:2, size = 10, part = "body")
dt.ft
```
\
\
